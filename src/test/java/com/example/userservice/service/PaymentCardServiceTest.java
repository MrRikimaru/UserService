...
class PaymentCardServiceTest {
  ...
  void createCard_ShouldThrowCardLimitExceededException_WhenUserHas5Cards() {
    ...
    // Act & Assert
    assertThrows(
        CardLimitExceededException.class, () -> paymentCardService.createCard(requestDTO, userId));
  ...
  void getAllCards_ShouldReturnPageOfCards() {
    ...
    Page<PaymentCardResponseDTO> result =
        paymentCardService.getAllCards("John", true, 1L, pageable);
  ...
  void updateCard_ShouldThrowDuplicateCardNumberException_WhenNumberExists() {
    ...
    when(paymentCardRepository.findById(cardId)).thenReturn(Optional.of(existingCard));
    when(paymentCardRepository.findByNumber("existing-number"))
        .thenReturn(Optional.of(duplicateCard));
  ...
  void updateCard_ShouldThrowIllegalArgumentException_WhenExpirationDateInPast() {
    ...
    // Act & Assert
    assertThrows(
        IllegalArgumentException.class, () -> paymentCardService.updateCard(cardId, requestDTO));
    verify(paymentCardRepository, never()).save(any(PaymentCard.class));
  }
