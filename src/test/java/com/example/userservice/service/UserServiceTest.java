...
class UserServiceTest {
  ...
  @Test
  void activateUser_ShouldCallRepository_WhenUserExists() {
    // Arrange
    Long userId = 1L;
  ...
  @Test
  void activateUser_ShouldThrowUserNotFoundException_WhenUserNotExists() {
    // Arrange
    Long userId = 999L;
  ...
  @Test
  void deactivateUser_ShouldCallRepository_WhenUserExists() {
    // Arrange
    Long userId = 1L;
    when(userRepository.existsById(userId)).thenReturn(true);
  ...
  @Test
  void deactivateUser_ShouldThrowUserNotFoundException_WhenUserNotExists() {
    // Arrange
    Long userId = 999L;
    when(userRepository.existsById(userId)).thenReturn(false);
  ...
  void getActiveUsersBornBefore_ShouldReturnFilteredUsers() {
    ...
    when(userRepository.findActiveUsersBornBefore(
            any(LocalDate.class), eq(true), any(Pageable.class)))
        .thenReturn(userPage);
    when(userMapper.toDTO(any(User.class))).thenReturn(responseDTO);
  ...
  @Test
  void getUserCards_ShouldThrowUserNotFoundException_WhenUserNotExists() {
    // Arrange
    Long userId = 999L;
    when(userRepository.existsById(userId)).thenReturn(false);

    // Act & Assert
    assertThrows(UserNotFoundException.class, () -> userService.getUserCards(userId));
    verify(paymentCardRepository, never()).findByUserId(anyLong());
  }
